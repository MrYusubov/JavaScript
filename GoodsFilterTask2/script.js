...
function filterByPrice(arr){
    return arr.filter((g)=>g.product_price>=50 && g.product_price<=200);
}
function filterByName(arr,str){
    return arr.filter((g)=>g.product_name.includes(str));
}
let list=document.getElementById('list');
let byPrice=filterByPrice(goods);
let byPriceName=filterByName(byPrice,"Kişi");
for(let i=0;i<byPriceName.length;i++){
    index=byPriceName[i];
