let goods = [
  ...
  function filterByName(arr,str){
    return arr.filter((g)=>g.store_name===str);
}
  let moda=document.getElementById('moda');
  let zerif=document.getElementById('zerif');
  let aliyev=document.getElementById('aliyev');
  let list=document.getElementsByClassName('list')
  moda.addEventListener('click',()=>{
    let byName=filterByName(goods,"Moda Dünyası");
    console.log(byName)
for(let i=0;i<byName.length;i++){
    index=byName[i];
    let li=document.createElement('li')
    li.innerHTML = `
    <li><span>Product Name: </span>${index.product_name}</li>
    <li><span>Product Description: </span>${index.product_description}</li>
    <li><span>Product Price: </span>${index.product_price}</li>
    <li><span>Store Name: </span>${index.store_name}</li>
    <li><span>Store Adress: </span>${index.store_address}</li>
    <li><span>Id: </span>${index.id}</li>
    `
    moda.appendChild(li)
}
  })

  zerif.addEventListener('click',()=>{
    let byName=filterByName(goods,"Zərif Moda");
    console.log(byName)
for(let i=0;i<byName.length;i++){
    index=byName[i];
    let li=document.createElement('li')
    ...
    <li><span>Id: </span>${index.id}</li>
    `
    zerif.appendChild(li)
}
  })

  aliyev.addEventListener('click',()=>{
    let byName=filterByName(goods,"Əliyev Moda Mərkəzi");
    console.log(byName)
for(let i=0;i<byName.length;i++){
    index=byName[i];
    let li=document.createElement('li')
