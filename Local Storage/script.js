...
goods.forEach(good => {
    ...
    itemDiv.innerHTML = `
        ...
        <p><b>Price:</b> ${good.product_price} AZN</p>
...
goodsList.addEventListener("click", event => {
    if (event.target.tagName === "BUTTON") {
        const goodId = event.target.getAttribute("data-id");
        const selectedGood = goods.find(g => g.id === goodId);
        const cart = JSON.parse(localStorage.getItem("cart"));
        cart.push(selectedGood);
        localStorage.setItem("cart", JSON.stringify(cart));
        alert(`${selectedGood.product_name} added to cart!`);
    }
});
